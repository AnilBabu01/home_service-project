...
import defaultUser from '../images/user/defaultUser.png';
import { useEditAdminProfileMutation } from '../redux/api';
const Profile = () => {
  const [isData, setisData] = useState<any>({});
  const { data } = useGetProfileQuery();
  const [editAdminProfile, { isSuccess }] = useEditAdminProfileMutation();
  ...
  const handleFileChange = async (
    ...
    formData.append('adminprofile', file);

    try {
      const response = await editAdminProfile({ formData }).unwrap();
      if ('data' in response) {
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            <div className="relative drop-shadow-2">
              <img
                src={isData?.profile ? isData?.profile : defaultUser}
                alt="profile"
          ...
          <div className="mt-4">
            ...
            <p className="font-medium">{isData?.adminType}</p>

...
import { width } from "@/src/utils/responsuve";
import { useGetRoomsQuery } from "@/lib/react-query/serverinstamce";
  ...
  const renderItem = ({ item, index }: any) => (
    ...
    >
      <View style={styles.userImageContainer}>
        ...
        <Image
          source={
            item?.user?.image ? { uri: item?.user?.image } : images.default10
          }
          resizeMode="contain"
      ...
      <View style={{ flexDirection: "row", width: SIZES.width - 104 }}>
        <View style={[styles.userInfoContainer]}>
          ...
          >
            ...
            {item?.user?.fullname}
             {console.log("ddd",item?.user?.fullname)}
          </Text>
          <Text style={styles.lastSeen}>{item?.messages[0]?.message}</Text>
        </View>
        <View
          style={{
            position: "absolute",
